...
static INLINE __m256i fold_mul_and_sum(__m256i partial, __m256i const_var) {
    partial = _mm256_shuffle_epi8(partial,
        _mm256_set_epi32(0x0f0e0100,
            ...
            0x07060504,
            0x03020100));
    partial = _mm256_permutevar8x32_epi32(
        partial, _mm256_set_epi32(7, 3, 6, 2, 5, 1, 4, 0));
    partial = _mm256_shuffle_epi8(partial,
...
static INLINE void compute_directions(__m128i lines[8], int32_t tmp_cost1[4]) {
    ...
    partial4 = _mm256_setr_m128i(_mm_slli_si128(lines[0], 14),
        _mm_srli_si128(lines[0], 2));
    tmp_avx2 = _mm256_setr_m128i(_mm_slli_si128(lines[1], 12),
        _mm_srli_si128(lines[1], 4));
    partial4 = _mm256_add_epi16(partial4, tmp_avx2);
    tmp = _mm_add_epi16(lines[0], lines[1]);
    partial5 =
        _mm256_setr_m128i(_mm_slli_si128(tmp, 10), _mm_srli_si128(tmp, 6));
    partial7 =
        _mm256_setr_m128i(_mm_slli_si128(tmp, 4), _mm_srli_si128(tmp, 12));
    partial6 = tmp;

    /* Partial sums for lines 2 and 3. */
    tmp_avx2 = _mm256_setr_m128i(_mm_slli_si128(lines[2], 10),
        _mm_srli_si128(lines[2], 6));
    partial4 = _mm256_add_epi16(partial4, tmp_avx2);
    tmp_avx2 = _mm256_setr_m128i(_mm_slli_si128(lines[3], 8),
        _mm_srli_si128(lines[3], 8));
    partial4 = _mm256_add_epi16(partial4, tmp_avx2);
    tmp = _mm_add_epi16(lines[2], lines[3]);
    tmp_avx2 =
        _mm256_setr_m128i(_mm_slli_si128(tmp, 8), _mm_srli_si128(tmp, 8));
    partial5 = _mm256_add_epi16(partial5, tmp_avx2);
    tmp_avx2 =
        _mm256_setr_m128i(_mm_slli_si128(tmp, 6), _mm_srli_si128(tmp, 10));
    partial7 = _mm256_add_epi16(partial7, tmp_avx2);
    partial6 = _mm_add_epi16(partial6, tmp);

    /* Partial sums for lines 4 and 5. */
    tmp_avx2 = _mm256_setr_m128i(_mm_slli_si128(lines[4], 6),
        _mm_srli_si128(lines[4], 10));
    partial4 = _mm256_add_epi16(partial4, tmp_avx2);
    tmp_avx2 = _mm256_setr_m128i(_mm_slli_si128(lines[5], 4),
        _mm_srli_si128(lines[5], 12));
    partial4 = _mm256_add_epi16(partial4, tmp_avx2);
    ...
    tmp_avx2 = _mm256_setr_m128i(_mm_slli_si128(lines[6], 2),
        _mm_srli_si128(lines[6], 14));
    partial4 = _mm256_add_epi16(partial4, tmp_avx2);
    ...
    partial4 = fold_mul_and_sum(
        partial4, _mm256_set_epi32(105, 120, 140, 168, 210, 280, 420, 840));
    partial7 = fold_mul_and_sum(
        partial7, _mm256_set_epi32(105, 105, 105, 140, 210, 420, 0, 0));
    partial5 = fold_mul_and_sum(
        partial5, _mm256_set_epi32(105, 105, 105, 140, 210, 420, 0, 0));
...
int32_t eb_cdef_find_dir_avx2(const uint16_t *img, int32_t stride, int32_t *var,
    ...
    for (i = 0; i < 8; i++) {
        ...
        lines[i] = _mm_sub_epi16(
            _mm_sra_epi16(lines[i], _mm_cvtsi32_si128(coeff_shift)), const_128);
    }
...
void eb_cdef_filter_block_4x4_8_avx2(uint8_t *dst, int32_t dstride,
    ...
    const int32_t *pri_taps =
        eb_cdef_pri_taps[(pri_strength >> coeff_shift) & 1];
    const int32_t *sec_taps =
        eb_cdef_sec_taps[(pri_strength >> coeff_shift) & 1];
    ...
    max = _mm256_max_epi16(
        _mm256_max_epi16(
            max, _mm256_andnot_si256(_mm256_cmpeq_epi16(p0, large), p0)),
    ...
    sum = _mm256_add_epi16(sum,
        _mm256_mullo_epi16(_mm256_set1_epi16(pri_taps[0]),
    ...
    sum = _mm256_add_epi16(sum,
        _mm256_mullo_epi16(_mm256_set1_epi16(pri_taps[1]),
            _mm256_add_epi16(p0, p1)));
    ...
    max = _mm256_max_epi16(
        _mm256_max_epi16(
            max, _mm256_andnot_si256(_mm256_cmpeq_epi16(p2, large), p2)),
        _mm256_andnot_si256(_mm256_cmpeq_epi16(p3, large), p3));
    min = _mm256_min_epi16(
        _mm256_min_epi16(_mm256_min_epi16(_mm256_min_epi16(min, p0), p1), p2),
        p3);
    ...
    sum =
        _mm256_add_epi16(sum, _mm256_cmpgt_epi16(_mm256_setzero_si256(), sum));
...
void eb_cdef_filter_block_8x8_8_avx2(uint8_t *dst, int32_t dstride,
    ...
    for (i = 0; i < 8; i += 2) {
        ...
        // sum += pri_taps[0] * (p0 + p1)
        sum = _mm256_add_epi16(
            sum, _mm256_mullo_epi16(pri_taps_0, _mm256_add_epi16(p0, p1)));
        ...
        // sum += pri_taps[1] * (p0 + p1)
        sum = _mm256_add_epi16(
            sum, _mm256_mullo_epi16(pri_taps_1, _mm256_add_epi16(p0, p1)));
        ...
        max = _mm256_max_epi16(
            ...
            _mm256_andnot_si256(_mm256_cmpeq_epi16(p3, large), p3));
        min = _mm256_min_epi16(
            _mm256_min_epi16(_mm256_min_epi16(_mm256_min_epi16(min, p0), p1),
                p2),
            p3);
        p0 = constrain16(p0, row, sec_strength_256, sec_damping);
        p1 = constrain16(p1, row, sec_strength_256, sec_damping);
        ...
        // res = row + ((sum - (sum < 0) + 8) >> 4)
        sum = _mm256_add_epi16(sum,
            _mm256_cmpgt_epi16(_mm256_setzero_si256(), sum));
    ...
    *sum = _mm256_add_epi16(
        *sum,
        _mm256_mullo_epi16(sec_taps,
            _mm256_add_epi16(_mm256_add_epi16(q0, q1),
                _mm256_add_epi16(q2, q3))));
}

void eb_cdef_filter_block_8x8_16_avx2(
    ...
    for (i = 0; i < 8; i += 2) {
        ...
        res = _mm256_add_epi16(row, res);
        res = _mm256_min_epi16(_mm256_max_epi16(res, min), max);
        _mm_storeu_si128((__m128i *)&dst[i * dstride],
            _mm256_castsi256_si128(res));
        _mm_storeu_si128((__m128i *)&dst[(i + 1) * dstride],
...
void eb_cdef_filter_block_avx2(uint8_t *dst8, uint16_t *dst16, int32_t dstride,
    ...
    int32_t sec_strength, int32_t dir,
    int32_t pri_damping, int32_t sec_damping,
    int32_t bsize, int32_t coeff_shift) {
    if (dst8) {
        if (bsize == BLOCK_8X8) {
...
void eb_copy_rect8_16bit_to_16bit_avx2(uint16_t *dst, int32_t dstride,
    ...
    for (i = 0; i < v; i++) {
        for (j = 0; j < (h & ~0x7); j += 8) {
            __m128i row = _mm_lddqu_si128((__m128i *)&src[i * sstride + j]);
            _mm_storeu_si128((__m128i *)&dst[i * dstride + j], row);
        }
        for (; j < h; j++)
            dst[i * dstride + j] = src[i * sstride + j];
    }
